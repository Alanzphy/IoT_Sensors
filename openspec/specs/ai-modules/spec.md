...
- The conversational assistant SHALL be enabled by default (`AI_ASSISTANT_ENABLED` defaults to `true` in code and compose) and works without Azure via the deterministic fallback; when set to `false`, `POST /api/v1/ai-assistant/chat` SHALL respond 503.
- Asynchronous reports SHALL be dormant by default (`AI_REPORTS_ENABLED` defaults to `false`; `POST /api/v1/ai-reports/generate` responds 503 while disabled).
- The AI report scheduler SHALL run as a compose service gated by `AI_REPORTS_SCHEDULER_ENABLED` (default false). No compose profiles are used.
